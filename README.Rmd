---
title: "Model to build the Ports database"
author: "Ricardo Lemos"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Model to build the Ports database}
...
Package `portsModel` was built during the 2018 Fishackathon, to address Challenge #9 ("Ports"). In short,
...
- name the port, based on the World Port Index database and the Geonames1000 database (a global database of cities and villages with more than 1000 habitants);
...
have 100m in meridional (i.e., latitudinal) resolution, and also 100m zonal (longitudinal) resolution at the equator. As we go poleward, the zonal resolution increases, meaning that we have to traverse more cells to cover the same 100m. For convenience, we place an upper limit of 5 on the number of cells traversed. This should not pose issues, since few ports exist beyond parallel 66 -- the Arctic and Antarctic circles.
...
Here we employ `unnamed_anchorages_20171120`, which is a dataset of anchorages provided by GFW for the 2018 Fishackathon. This dataset contains the latitudes, longitudes, and Google-S2 ID code of 102,974 anchorages, defined by Global Fishing Watch. We also use a database of ports -- 
the [World Port Index](https://msi.nga.mil/MSISiteContent/StaticFiles/NAV_PUBS/WPI/WPI_Shapefile.zip) --, which is a tabular listing of thousands of ports throughout the world, describing their location, characteristics, known facilities, and available services. Lastly, we use `cities1000.zip`, which is a 
[GeoNames Gazetteer dataset](http://download.geonames.org/export/dump/readme.txt) that provides all cities in the world with more that 1000 habitants.
...
```{r, cache = TRUE}
anchorages = points2MatrixClass(earthGeo, unnamed_anchorages_20171120, 'lat', 'lon', 's2id', NULL)
wpi = points2MatrixClass(earthGeo, wpiDB, 'LATITUDE', 'LONGITUDE', 'PORT_NAME', 'COUNTRY')
cities1000 = points2MatrixClass(earthGeo, citiesDB, 'latitude', 'longitude', 'name', 'country')
```
...
```{r, cache = TRUE}
clusters = clustersClass(earthGeo, anchorages)
clusters$id
```
...
```{r, cache = TRUE}
polygons = polygonsClass(clusters)
...
`anchTypes`, provided by GFW during Fishackathon 2018.

```{r, cache = TRUE}
ports = portsClass(polygons, wpi, cities1000, anchTypes)
```
